...
// al pulsar el boton examen se hace
/*
document.getElementById('examen').addEventListener('click', function() {
});
*/

// hago una funcion para obtener los medicos perteecientes a una provincia
function obtenerMedicosProvincia() {
    
    rest.get(`/api/provincia/${provincia}`, (status, response) => {
        if (status === 200) {
            console.log('Medicos de la provincia:', response);
            // Ahora puedes mostrar los médicos en un select o en una lista
        } else {
            console.error('Error al obtener los médicos de la provincia:', response.error);
        }
    });
}
